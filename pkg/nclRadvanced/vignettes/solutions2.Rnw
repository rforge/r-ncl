...
\begin{enumerate}
\item Create an \cc{.Rprofile} file. Put in the line
<<message=FALSE>>=
cat("Successfully loaded .Rprofile at", date(), "\n")
@
\noindent Restart R. Does the welcome message appear. 
\item Try adding my suggestions to your \cc{.Rprofile}
\end{enumerate}
...
hist(cc)
@
\item Create a \cc{[} method for the \cc{cohort} class. This method should return a \cc{cohort} object, but with the relevant rows sub setted. For example, if \c{cc} was a cohort object, 
<<eval=FALSE>>=
cc[1:3,]
@
...
\item Create a verb"[<-" method for the \cc{cohort} class. This method should allow us to replace values in the \cc{details} data frame, i.e.
<<eval=FALSE>>=
cc[1,1] = 10
@
...
\item Create a verb"[<-" method for the \cc{cohort} class. This method should allow us to replace values in the \cc{details} data frame.
