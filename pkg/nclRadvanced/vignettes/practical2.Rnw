...
show = TRUE
results = ifelse(show, "markup", "hide")
@
...
\title{Advanced R: \Sexpr{ifelse(show, "solutions","practical")}  2} 
...
\begin{enumerate}
\item Following the cohort example in the notes, suppose we want to create method called \cc{mean}. 
\begin{itemize}
\item List all the S3 methods associated with the \cc{mean} function.
<<echo=show, results=results>>=
methods("mean")
@
