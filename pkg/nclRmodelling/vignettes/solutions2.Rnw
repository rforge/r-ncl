...
\item Calculate the sample correlation coefficient $r$. Perform a hypothesis
  test, where $H_0: \rho =0$ and $H_1: \rho \ne 0$.\sidenote{Where $\rho$ is the
    population correlation coefficient.} Compare this $p$-value to the $p$-value
...
<<eval=FALSE, echo=echo, results=results>>=
text(5, 130, "r=0.983")
@
...
\begin{table}[h]
  ...
  \begin{tabular}{@{}l llllll llllll@{}}
    ...
    \bottomrule
  \end{tabular}
  \caption{Twelve measurements of from a study of temperature on yield.}\label{T2}
\end{table}
\end{enumerate}
...
\item \newthought{The data} (below) are from 101 consecutive patients attending a combined
...
\noindent and make the a \cc{Surv} object

<<results="hide">>=
Surv(heart$start, heart$stop, heart$event) 
@ 

\noindent Why are three arguments in the above function? Construct a
