...
However, we want the object of normalising the data using different transformations. In particular, we want the ability to pass arbaritary transformation to the \cc{stat\_ana} function.
\begin{itemize}
...
@
\item Using \cc{is.function} to test whether a function has been passed to \cc{trans}, transform the vectors \cc{x} and \cc{y} when appropriate. For example, 
<<eval=FALSE>>=
